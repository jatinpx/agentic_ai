...
export default function LinkedInPage() {
  ...
  const threadDetailCacheRef = useRef<Record<string, HistoryPost>>({});
  ...
  const handleSelectThread = useCallback(async (threadId: string) => {
    const cached = threadDetailCacheRef.current[threadId];
    const isSameThread = selectedThreadId === threadId;

    if (isSameThread && cached) {
      setSelectedHistoryPost(cached);
      return;
    }
    ...
    try {
      const res = await fetch(`${API_BASE}/linkedin/posts/${threadId}`, { cache: "no-store" });
      if (!res.ok) {
        throw new Error("Failed to fetch thread details");
      }
      const data = (await res.json()) as HistoryPost;
      threadDetailCacheRef.current[threadId] = data;
      setSelectedHistoryPost(data);
    } catch {
      setStatusMessage("❌ Failed to load selected thread details.");
    }
  }, [selectedThreadId]);
  ...
  const parseIterationNumber = useCallback((value: unknown): number | null => {
  ...
  const getIterationFromPipeline = useCallback((): number | null => {
    ...
    return null;
  }, [parseIterationNumber, pipelineStatus.events, pipelineStatus.metadata]);
  ...
  }, [getIterationFromPipeline, iteration]);
  ...
  const pipelineCard = (
    <GlassCard className="p-5">
      ...
      <div className="mt-4 flex flex-wrap gap-3">
        <StatPill label="Viral Score" value={viralScore.toFixed(1)} />
  ...
  return (
    <div className="flex flex-col h-screen w-full">
      ...
      {/* Main flex row: left nav, center content, right detail */}
      <div className="flex flex-1 min-h-0 w-full">
        {/* Left thread panel */}
        <ThreadPanel baseUrl={API_BASE} onSelectThread={handleSelectThread} selectedThreadId={selectedThreadId || undefined} />
        {/* Center main content */}
        <div className="flex-1 flex flex-col overflow-y-auto">
          <div className="mx-auto w-full max-w-6xl flex-col gap-5 py-5 px-4">
            <motion.section
        ...
        <AnimatePresence>
          {selectedHistoryPost && (
            <motion.div className="fixed inset-y-0 right-0 z-50 flex justify-end bg-black/35" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={closeSelectedThread}>
              <motion.aside
                className="relative h-full rounded-none border-l border-[var(--glass-border)] bg-[var(--background)] p-6 shadow-2xl"
                style={{ width: `${historyPanelWidth}px`, maxWidth: "78vw" }}
                initial={{ x: 480 }}
                animate={{ x: 0 }}
                exit={{ x: 460 }}
              ...
              >
                ...
                <div className="mb-4 flex items-start justify-between gap-3">
                  ...
                  <button type="button" onClick={closeSelectedThread} className="ghost-button px-3 py-1.5 text-xs">
                    Close
                  </button>
                </div>
